...
use roost_ui::drawable::{Color, ShapeType, Image};
use roost_ui::{Context, Component};

use roost_ui::layouts::{Padding, Size, Offset, Stack, Bin};
...
impl QRCode {
    pub fn new(ctx: &mut Context, data: &str) -> Self {
        let app_icon = ctx.get::<PelicanUI>().get().0.theme().brand.app_icon.clone();
        let qr_size = 300.0;
        let logo_size = 64.0;
